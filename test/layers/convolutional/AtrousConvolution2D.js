  ...
  const testParams = [
    {
      inputShape: [5, 5, 2],
      kernelShape: [4, 3, 3],
      attrs: { activation: 'linear', borderMode: 'valid', subsample: [1, 1], atrousRate: [2, 2], dimOrdering: 'tf', bias: true }
    },
    {
      inputShape: [5, 5, 2],
      kernelShape: [4, 3, 3],
      attrs: { activation: 'linear', borderMode: 'valid', subsample: [1, 1], atrousRate: [2, 2], dimOrdering: 'tf', bias: false }
    },
    {
      inputShape: [7, 7, 2],
      kernelShape: [4, 3, 3],
      attrs: { activation: 'relu', borderMode: 'valid', subsample: [1, 1], atrousRate: [3, 3], dimOrdering: 'tf', bias: true }
    },
    {
      inputShape: [4, 8, 3],
      kernelShape: [3, 4, 4],
      attrs: { activation: 'relu', borderMode: 'same', subsample: [1, 1], atrousRate: [2, 2], dimOrdering: 'tf', bias: true }
    },
    {
      inputShape: [8, 8, 2],
      kernelShape: [4, 3, 3],
      attrs: { activation: 'relu', borderMode: 'same', subsample: [1, 1], atrousRate: [4, 4], dimOrdering: 'tf', bias: true }
    }
  ]
  ...
  *********************************************************/
  ...
  describe('GPU', function () {
    ...
    testParams.forEach(({ inputShape, kernelShape, attrs }, i) => {
      ...
      it(title, function () {
        console.log(`\n%c${title}`, styles.h3)
        let testLayer = new layers.AtrousConvolution2D(Object.assign({ nbFilter, nbRow, nbCol }, attrs, { gpu: true }))
        testLayer.setWeights(TEST_DATA[key].weights.map(w => new KerasJS.Tensor(w.data, w.shape)))
        let t = new KerasJS.Tensor(TEST_DATA[key].input.data, TEST_DATA[key].input.shape)
        console.log('%cin', styles.h4, stringifyCondensed(t.tensor))
        const startTime = performance.now()

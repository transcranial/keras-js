describe('pooling layer: GlobalAveragePooling3D', function() {
  ...
  before(function() {
    console.log('\n%cpooling layer: GlobalAveragePooling3D', styles.h1)
  })
  ...
  describe('CPU', function() {
    ...
    testParams.forEach(({ inputShape, attrs }, i) => {
      const key = `pooling.GlobalAveragePooling3D.${i}`
      const title = `[${key}] [CPU] test: ${inputShape} input, data_format=${attrs.data_format}`

      it(title, function() {
        console.log(`\n%c${title}`, styles.h3)
        let testLayer = new layers.GlobalAveragePooling3D(attrs)
  ...
  describe('GPU', function() {
    ...
    testParams.forEach(({ inputShape, attrs }, i) => {
      ...
      it(title, function() {
        console.log(`\n%c${title}`, styles.h3)
        let testLayer = new layers.GlobalAveragePooling3D(Object.assign(attrs, { gpu: true }))
        let t = new KerasJS.Tensor(TEST_DATA[key].input.data, TEST_DATA[key].input.shape)
        console.log('%cin', styles.h4, stringifyCondensed(t.tensor))
        const startTime = performance.now()
        t = testLayer.call(t)

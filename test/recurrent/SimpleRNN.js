...
describe('recurrent layer: SimpleRNN', function () {
  ...
  const testParams = [
    {
      inputShape: [3, 6],
      attrs: { outputDim: 4, activation: 'tanh' }
    },
    {
      inputShape: [8, 5],
      attrs: { outputDim: 5, activation: 'sigmoid' }
    }
  ]
  ...
  *********************************************************/
  ...
  describe('CPU', function () {
    ...
    testParams.forEach(({ inputShape, attrs }, i) => {
      ...
      const title = `[${key}] [CPU] test: ${inputShape[0]}x${inputShape[1]} input, activation='${attrs.activation}'`
  ...
  *********************************************************/
  ...
  describe('GPU', function () {
    ...
    testParams.forEach(({ inputShape, attrs }, i) => {
      const key = `recurrent.SimpleRNN.${i}`
      const title = `[${key}] [GPU] test: ${inputShape[0]}x${inputShape[1]} input, activation='${attrs.activation}'`

  ...
  *********************************************************/
  ...
  describe('CPU', function () {
    ...
    testParams.forEach(({ wrappedLayer, inputShape, attrs, wrappedLayerAttrs }, i) => {
      ...
      it(title, function () {
        console.log(`\n%c${title}`, styles.h3)
        let testLayer = new layers.Bidirectional(Object.assign(attrs, { layer: new layers[wrappedLayer](wrappedLayerAttrs) }))
  ...
  *********************************************************/
  ...
  describe('GPU', function () {
    ...
    testParams.forEach(({ wrappedLayer, inputShape, attrs, wrappedLayerAttrs }, i) => {
      ...
      it(title, function () {
        console.log(`\n%c${title}`, styles.h3)
        let testLayer = new layers.Bidirectional(Object.assign(attrs, { layer: new layers[wrappedLayer](wrappedLayerAttrs), gpu: true }))
        testLayer.setWeights(TEST_DATA[key].weights.map(w => new KerasJS.Tensor(w.data, w.shape)))
        let t = new KerasJS.Tensor(TEST_DATA[key].input.data, TEST_DATA[key].input.shape)
        console.log('%cin', styles.h4, stringifyCondensed(t.tensor))
        const startTime = performance.now()

...
export default class WebGLConv2D extends WebGLLayer {
  ...
  _bindInputTexturesInputTransform(input, indexMappingRow, indexMappingCol) {
    ...
    this._bindInputTexture(
      ...
      input.texture,
      gl.TEXTURE0,
      WebGLConv2D.INPUT_TEXTURE_NAME
    );
    this._bindInputTexture(
      this.inputTransformProgram,
  ...
  _bindInputTexturesMain(input, weights, bias) {
    ...
    this._bindInputTexture(
      ...
      weights.texture,
      gl.TEXTURE1,
      WebGLConv2D.WEIGHTS_TEXTURE_NAME
    );
    this._bindInputTexture(
      this.mainProgram,
      bias.texture,
      gl.TEXTURE2,
      WebGLConv2D.BIAS_TEXTURE_NAME
    );
  }
  ...
  _bindUniformsInputTransform(input, indexMappingRow) {
    ...
    gl.uniform1i(
      gl.getUniformLocation(
        this.inputTransformProgram,
        WebGLConv2D.INPUT_ROWS_UNIFORM_NAME
      ),
      nbPatches
    );
    gl.uniform1i(
      gl.getUniformLocation(
        this.inputTransformProgram,
        WebGLConv2D.INPUT_COLS_UNIFORM_NAME
      ),
      patchLen
    );
    gl.uniform1i(
      gl.getUniformLocation(
        this.inputTransformProgram,
        WebGLConv2D.OUTPUT_COLS_UNIFORM_NAME
      ),
      nbFilter
    );
    gl.uniform1i(
      gl.getUniformLocation(
        this.inputTransformProgram,
        WebGLConv2D.INPUT_COL_PAD_UNIFORM_NAME
      ),
      inputColPad
    );
    gl.uniform1i(
      gl.getUniformLocation(
        this.inputTransformProgram,
        WebGLConv2D.OUTPUT_COL_PAD_UNIFORM_NAME
      ),
      outputColPad
    );
  }
  ...
  _bindUniformsMain(input, weights, activation) {
    ...
    if (activation === 'relu') {
      gl.uniform1i(
        gl.getUniformLocation(
          this.mainProgram,
          WebGLConv2D.RELU_ACTIVATION_UNIFORM_NAME
        ),
        1
      );
    }
  ...
  transformInput(input, indexMappingRow, indexMappingCol) {
    ...
    const inputTransformed = new weblas.pipeline.Tensor(
      indexMappingRow.shape,
      null
    );

    this._bindInputTexturesInputTransform(
      input,
      indexMappingRow,
      indexMappingCol
    );
